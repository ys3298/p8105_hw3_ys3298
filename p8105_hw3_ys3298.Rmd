...
Comment: This figure has so many states information, so it's not so clearly to see. We notice that most of the states fluctuate from 2002 to 2010.
...
Comment: From the Plot, we can see the distribution of data value for reponses among locations. "Poor" was the lowest and "Very good" and "Excellent" and relatively high. There is difference in different locations. Form 2006 to 2010, the distribution changed a little bit.
...
accel =
  ...
  arrange(week, day_id) %>%
  mutate(starts_with("activity_"), round(starts_with("activity_"))) %>% 
    mutate_if(is.double, as.integer)

```

Comment: For this resulting dataset, there are `r nrow(accel)` observations and `r ncol(accel)` variables. It also includes the week, day ID, weekend or weekday information. I round all data into integer in order to see clearly.
...
Comment: I **rearrange the day_ id (still named day_id)** in oreder to put it into right order. This table in not clear to see the trend. From the plot, we can see in week1 and week2, the sum for each day increase from Monday to Sunday, however, in week3, week4 and week5, the trends are not so aparent.
...
Comment: Based on this graph, we can see the activity for 1st week is relatively low and the activity for week 2 and 3 is relatively high. In the first week, the activity for Monday, Tuesday, Wednesday, Thursday is very low compared with others.
