...
From the plot, we can clearly see the rank of number of items ordered in each aisle. The orders from fresh vegetables are the most and  butter is the least within those whose order is more than 10000.
...
From this table, we can clearly see the mean hour of the day at which pink lady apples and coffee ice cream are ordered on each day of the week. Coffee ice cream is ordered most on Tuesday and least on Friday.
...
location2010 =
  ...
  arrange(total_location)
```
In 2002, `r location2002$locationabbr` were observed at 7 or more locations. Total: `r length(location2002$locationabbr)`
...
From the Plot, we can see the distribution of data value for reponses among locations. "Poor" was the lowest and "Very good" and "Excellent" and relatively high. There is difference in different locations. Form 2006 to 2010, the distribution changed a little bit.
...
For this resulting dataset, there are `r nrow(accel)` observations and `r ncol(accel)` variables. It also includes the week, day ID, weekend or weekday information. I round all data into integer in order to see clearly.
...
ggplot(plotdf) + geom_linerange(aes(x=index,ymax=value,ymin=0,colour=day)) + scale_x_continuous("weeks", breaks = seq(5040, 50400 - 5040, 1440*7),labels=c(1:5)) + labs(title = "activity in 24 hours")
  
```


Based on this graph, we can see the activity for 1st week is relatively low and the activity for week 2 and 3 is relatively high. In the first week, the activity for Monday, Tuesday, Wednesday, Thursday is very low compared with others.
