...
table %>%
  #group_by(week) %>%
  select(week, day, day_id, sum) %>%
  ggplot(aes(x = week, y = sum, color = day)) +
  geom_point(size = 4, alpha = .5) + geom_line() +
  labs(
      title = "24-hour activity by week",
      x = "Week",
      y = "Sum of a day"
    ) +
  viridis::scale_color_viridis(
...
table %>%
 ...
 plotdf <- accel %>% 
   pivot_longer(cols=starts_with("activity_"), names_prefix = ("activity_"), ) %>% 
   mutate(name=as.integer(name)) %>% mutate(index=name+1440*day_id-1440)
 ggplot(plotdf) + geom_linerange(aes(x=index,ymax=value,ymin=0,colour=day))
