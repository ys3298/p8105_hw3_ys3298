...
Comment: This figure has so many states information, so it's not so clearly to see. We notice that most of the states fluctuate from 2002 to 2010. Also, I do notice that there're a few NAs after take the average.
...
# or in this way(I keep this chunk, but don't run it.):
c =  brfss_OH %>%
  filter(locationabbr == "NY" & year == "2006" ) %>%
  select(locationdesc, response, data_value) %>%
  group_by(response) %>%
